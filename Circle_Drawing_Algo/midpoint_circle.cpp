...
///Midpoint circle drawing algorithm function
void mid_point_circle(PT center, int radious)
{
    PT a;
    a.x = 0;
    a.y = radious;

    int decision_parameter = 1-radious; //Although the equation is 1.25-r but for practical purpose it should suffice.
    ...
    while(a.x<=a.y)
    {
        ...
        if(decision_parameter<0)
            decision_parameter = decision_parameter + (2 * a.x) + 3;
        else
        {
            a.y--;
            decision_parameter = decision_parameter + (2 * a.x) - (2 * a.y) + 5;
        }
...
int main()
{
    ...
    //mid_point_circle(PT(50,50),100);
    mid_point_circle(PT(0,0),100);
    getchar();
